...
public class RadialMenuWidget extends View {
    ...
    public interface OnMenuItemClick {
        void onItemSelected(CategoryResponse category);
    }
    ...
    private List<RadialMenuEntryIFace> menuEntries = new ArrayList<>();
    private RadialMenuEntryIFace centerCircle = null;

    private OnCenterClick onCenterClickListener;
    private OnMenuItemClick onMenuItemClickListener;


    public RadialMenuWidget(Context context, List<CategoryResponse> categories, OnMenuItemClick onMenuItemClickListener) {
        super(context);
        ...
        for (CategoryResponse category : categories) {
            if(category != null && category.getName() != null) {
                menuEntries.add(new RadialMenuWidget.RadialMenuEntry("wtf", category.getName().replace(",", "\n  \n").toUpperCase()));
            }
        }
    ...
    public boolean setCenter(String name, int icon, OnCenterClick onCenterClick) {
        centerCircle = new RadialMenuWidget.RadialMenuEntry("wtf", name.toUpperCase()).setIcon(icon);
    ...
    private void createRadialMenuWidget() {  // это конструктор
    ...
    public interface RadialMenuEntryIFace {
        String getName();
        ...
        List<RadialMenuEntryIFace> getChildren();
    }


    public static class RadialMenuEntry implements RadialMenuEntryIFace {

        private String name, label;
        private int icon;

        public RadialMenuEntry(String name, String label) {
            this.name = name;
            this.label = label;
        }
        ...
        @Override
        public List<RadialMenuEntryIFace> getChildren() {
            return null;
        }

        public RadialMenuEntry setName(String name) {
            this.name = name;
            return this;
        }

        public RadialMenuEntry setLabel(String label) {
            this.label = label;
            return this;
        }

        public RadialMenuEntry setIcon(int icon) {
            this.icon = icon;
            return this;
        }
    }
    ...
    /*========================================  ГАДОСТЬ (кроме onTouchEvent)  ========================================*/


    private float screen_density = getContext().getResources().getDisplayMetrics().density;

    private int defaultCenterColor = Color.parseColor("#e35331");    //default color of center piece
    private int defaultColor = Color.parseColor("#20262c");    //default color of wedge pieces
    private int defaultAlpha = 255;                        //transparency of the colors, 255=Opague, 0=Transparent
    private int wedge2Color = Color.parseColor("#31373d");    //default color of wedge pieces
    ...
    private int selectedColor = Color.parseColor("#31373d");    //color to fill when something is selected
    private int selectedAlpha = 255;                        //transparency of fill when something is selected

    private int disabledColor = Color.rgb(34, 96, 120);    //color to fill when something is selected
    private int disabledAlpha = 255;                        //transparency of fill when something is selected
    ...
    private int headerBackgroundColor = Color.parseColor("#474b51"); //color of header background
    //
    ...
    private RadialMenuEntryIFace wedge2Data = null;        //Keeps track off which menuItem data is being used for the outer ring

    private int MinSize = scalePX(78);                //Radius of inner ring size
    private int MaxSize = scalePX(172);                //Radius of outer ring size
        ...
        if (state == MotionEvent.ACTION_DOWN) {
            ...
            if (inCircle == true) {
                ...
                if (onCenterClickListener != null) {
                    onCenterClickListener.centerSelected();
                }
                    ...
                    if (f == selected) {
                        ...
                        if (enabled != null) {
                            ...
                            if (onMenuItemClickListener != null) {
                                onMenuItemClickListener.onItemSelected(categories.get(i));
                            }
                ...
                for (int i = 0; i < Wedges2.length; i++) {
                    ...
                    if (f == selected2) {
//                        if (onMenuItemClickListener != null) {
//                            onMenuItemClickListener.onItemSelected(i);
//                        }
                        animateOuterIn = true;  //sets Wedge2Shown = false;
                        enabled = null;
                        selected = null;
        ...
        for (int i = 0; i < Wedges.length; i++) {
            ...
            if ((menuEntries.get(i).getIcon() != 0) && (menuEntries.get(i).getLabel() != null)) {
                ...
                for (int j = 0; j < stringArray.length; j++) {
                    paint.getTextBounds(stringArray[j], 0, stringArray[j].length(), rect);
                    float textLeft = rf.centerX() - rect.width() / 2;
                    textBottom = textBottom + (rect.height() + 3/*6)*/)+(rect.height() /2);
                    c.drawText(stringArray[j], textLeft - rect.left, textBottom - rect.bottom - (rect.height() /2), paint);
                }
            }
...
//		}
        ...
        if (inCircle == true) {
            ...
            paint.setColor(defaultCenterColor);
            paint.setAlpha(defaultAlpha);
            paint.setStyle(Paint.Style.FILL);
            c.drawCircle(xPosition, yPosition, cRadius, paint);
        }
        ...
        if ((centerCircle.getIcon() != 0) && (centerCircle.getLabel() != null)) {
            ...
            for (int j = 0; j < stringArray.length; j++) {
                ...
                c.drawText(stringArray[j], textLeft - rectText.left, (textBottom - rectText.bottom) + scalePX(4), paint);
            }
...
//        startAnimation(spriteAnimation);
    ...
    private void determineOuterWedges(RadialMenuEntryIFace entry) {
